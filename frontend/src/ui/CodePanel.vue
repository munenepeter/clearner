...
<template>
  <div class="h-full flex flex-col bg-card rounded-xl shadow-sm border border-border overflow-hidden">
    <!-- Header -->
    <div class="flex items-center justify-between bg-gradient-to-r from-muted to-muted/80 px-4 py-3 border-b border-border">
    ...
    <!-- Code Content -->
    <div 
      class="flex-1 overflow-auto bg-muted/30 transition-all duration-300"
      :class="{ 'opacity-100': visible, 'opacity-0': !visible }"
    >
      <div v-if="visible && code" class="flex h-full">
        <!-- Line numbers -->
        <div class="flex-shrink-0 py-4 px-3 text-right text-muted-foreground text-sm font-mono select-none bg-card/50 border-r border-border">
        ...
        <div class="flex-1 overflow-auto">
          <pre class="py-4 px-4 text-sm font-mono leading-6 text-foreground whitespace-pre"><code class="block">{{ code }}</code></pre>
        </div>
      </div>
      ...
      <div v-else class="h-full flex items-center justify-center p-8">
        <div class="text-center px-6 py-8 max-w-sm">
          <svg class="w-14 h-14 text-muted-foreground/50 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
